...
public class TestMergeSelectPerf {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length < 6) {
			System.out
					.println("input param,format: [jdbcurl] [user] [password]  [threadpoolsize]  [executetimes] [limit] ");
			return;
		}
		...
		long limit = Integer.parseInt(args[5]);
		...
		for (int i = 0; i < threadCount; i++) {
			try {
				...
				TravelRecordMergeJob job = new TravelRecordMergeJob(con,
						limit, executetimes,finshiedCount,failedCount);
